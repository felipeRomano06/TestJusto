    ...
    static func make<A:Encodable, T:Codable>(url:String, metodo:Metodo, parametros:A?, headers: [String:String]? = nil, completion: @escaping (T?,CodeResponse?) -> ()){
        ...
        if metodo == .get {
            if let contenido = parametros {
                ...
                do {
                    let contenidoDict = try contenido.asDictionary()
                    print(contenido)
                    print("")
                    
                    for (key, value) in contenidoDict {
                        var valStr = ""
                        
                        if let result_number = value as? Int{
                            valStr = String(result_number)
                        }
                       
                        sParams += key + "=" + (value as? String ?? valStr)  + "&"
                    }
                } catch{
                    completion(nil,CodeResponse.bad_decodable)
                    return
                }
            }
        ...
        if !sParams.isEmpty{
            sParams = "?" + sParams
            if(sParams.hasPrefix("&")){
                sParams.removeLast()
            }
        }
        
        let newURL = url + sParams
        
        print("newURL --->", newURL)
        
        
        // valida que la url tenga un formato correcto
        guard let urlForRequest = URL(string: newURL) else {
            print("WRONG URL \(url)")
